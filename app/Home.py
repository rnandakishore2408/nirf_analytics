"""NIRF Analytics — Saveetha Engineering College. Entry page (overview)."""
...
from common import C_MUTED, C_PRIMARY, PARAMS, card, inject_css, load_json, rankings, saveetha_history
...
with c3:
    s = pred.get("saveetha", {})
    card("Saveetha est. score 2026", f"{s.get('total_score', {}).get('median', '–')}", f"80% range {s.get('total_score', {}).get('p10', '–')} – {s.get('total_score', {}).get('p90', '–')}")
with c4:
    card("Most likely 2026 band", s.get("most_likely_band", "–"), f"P(top 100) = {100 * s.get('band_probabilities', {}).get('top 100', 0):.0f}%")
with c5:
...
| **Live Data Entry** | Staff enter current-year numbers; the model re-scores instantly with the live values |
