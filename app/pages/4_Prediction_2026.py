...
from common import C_PRIMARY, PARAMS, card, inject_css, load_json, q, rankings

st.set_page_config(page_title="Prediction 2026", page_icon="🔮", layout="wide")
inject_css()
...
if not pred:
    ...
    st.stop()
s = pred["saveetha"]; thr = pred["thresholds"]; val = pred["top100_model_validation"]

st.caption(f"Run at {pred['run_at']} · based on Saveetha's NIRF {s['submission_year_used']} filing and rankings 2017-2025")
...
with left:
    ...
    st.markdown(f"**Estimated parameter scores (2026 filing):** " + " · ".join(f"{k.upper()} {v}" for k, v in s["param_estimates"].items())
...
with right:
    ...
    st.dataframe(pd.DataFrame({"year": list(thr["cutoff_history"]), "rank-100 cut-off": list(thr["cutoff_history"].values())}).T, use_container_width=True)
...
with m1:
    ...
    st.dataframe(pd.DataFrame([{"parameter": k.upper(), "n": v["n"], "CV R²": v["cv_r2"], "CV MAE": v["cv_mae"], "CV RMSE": v["cv_rmse"],
                                "top drivers": ", ".join(list(v["importance"])[:3])} for k, v in pr.items()]), hide_index=True, use_container_width=True)
...
with m2:
    ...
    st.dataframe(pd.DataFrame([{"validation year": val["val_year"], "institutes": val["n"], "MAE": val["mae"], "RMSE": val["rmse"],
                                "naive 'no change' MAE": val["naive_mae_no_change"], "rank order Spearman (top 100)": val["rank_spearman_top100"]}]), hide_index=True, use_container_width=True)
...
st.dataframe(top[["pred_rank_2026", "rank_2025", "change", "name", "state", "score", "pred_score_2026"]].rename(columns={"score": "score_2025"}).head(100)
             .style.format({"score_2025": "{:.2f}", "pred_score_2026": "{:.2f}"}), hide_index=True, use_container_width=True, height=500)
...
st.dataframe(q("SELECT run_at, model, target_year, name, pred_score, pred_rank, pred_band, score_low, score_high FROM predictions WHERE institute_id='IR-E-C-16590' ORDER BY run_at DESC"),
